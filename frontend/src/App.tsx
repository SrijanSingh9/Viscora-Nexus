...
function App() {
  ...
  // Generate Advanced Deep Analysis (SWOT + WWW + 5 Whys + Quote)
  ...
  // Format Markdown (Bold, Italic, Newlines) to safe HTML
  ...
  const getLifeGrid = () => {
    ...
    return days.map((dayOffset, i) => {
      ...
      let activeColorClass = theme === 'dark' 
        ? (count === 1 ? 'bg-[#5EEAD4]/60 shadow-[0_0_8px_rgba(94,234,212,0.4)]' : count >= 2 ? 'bg-[#5EEAD4] shadow-[0_0_12px_rgba(94,234,212,0.8)] scale-110' : 'bg-[#2A3040] opacity-50 hover:opacity-80')
        : (count === 1 ? 'bg-[#6366F1]/60 shadow-[0_0_8px_rgba(99,102,241,0.4)]' : count >= 2 ? 'bg-[#6366F1] shadow-[0_0_12px_rgba(99,102,241,0.8)] scale-110' : 'bg-[#E3E6EF] hover:bg-[#D1D5DB]');
      
      return <div key={i} title={tooltip} className={`w-3 h-3 sm:w-4 sm:h-4 rounded-sm transition-all duration-300 hover:scale-125 cursor-help ${activeColorClass}`} />;
    });
  };
    ...
    <div className={`relative min-h-screen transition-colors duration-700 font-['Inter',sans-serif] selection:bg-[#5EEAD4] selection:text-[#0F1117] ${
      ...
      {token && profileData && (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
          {isChatOpen && (
            <div className={`mb-4 w-[340px] sm:w-[400px] h-[500px] flex flex-col rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 animate-fade-in-up border ${theme === 'dark' ? 'bg-[#161A23] border-[#2A3040]' : 'bg-white border-[#E3E6EF]'}`}>
              <div className={`p-4 flex justify-between items-center border-b ${theme === 'dark' ? 'bg-[#1D2230] border-[#2A3040]' : 'bg-[#F7F8FB] border-[#E3E6EF]'}`}>
                ...
                <button onClick={() => setIsChatOpen(false)} className={`p-1 rounded-md hover:bg-black/10 dark:hover:bg-white/10 transition-colors`}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
              </div>
              <div ref={chatScrollRef} className="flex-1 p-4 overflow-y-auto flex flex-col gap-4">
                {chatMessages.map((msg, idx) => (
                  <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[80%] p-3 rounded-2xl ${msg.role === 'user' ? (theme === 'dark' ? 'bg-[#5EEAD4]/20 text-[#E6EAF2] rounded-tr-sm' : 'bg-[#6366F1]/10 text-[#1F2937] rounded-tr-sm') : (theme === 'dark' ? 'bg-[#2A3040] text-[#A4A9B6] rounded-tl-sm' : 'bg-[#F1F3F9] text-[#4B5563] rounded-tl-sm')}`}>
                      <p className="text-sm leading-relaxed whitespace-pre-wrap" dangerouslySetInnerHTML={formatMarkdownText(msg.text)}></p>
                    </div>
                  </div>
                ))}
                {isNexusTyping && (
              ...
              <div className={`p-3 border-t flex flex-col gap-2 ${theme === 'dark' ? 'bg-[#1D2230] border-[#2A3040]' : 'bg-[#F7F8FB] border-[#E3E6EF]'}`}>
                <div className="flex gap-2">
                  ...
                  <button onClick={handleSendChatMessage} disabled={!chatInput.trim()} className={`p-2.5 rounded-xl transition-all duration-300 disabled:opacity-50 ${theme === 'dark' ? 'bg-[#5EEAD4] text-[#0F1117] hover:bg-[#A78BFA]' : 'bg-[#6366F1] text-white hover:bg-[#22D3EE]'}`}>
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"></path></svg>
                  </button>
                </div>
                <button onClick={handleSaveChatToMemory} disabled={chatMessages.length <= 1 || isAnalyzing} className={`w-full py-2 rounded-xl text-xs uppercase tracking-wider font-medium transition-all duration-300 border ${theme === 'dark' ? 'border-[#2A3040] text-[#A4A9B6] hover:bg-[#5EEAD4]/10 hover:text-[#5EEAD4] hover:border-[#5EEAD4]/30 disabled:opacity-30' : 'border-[#E3E6EF] text-[#6B7280] hover:bg-[#6366F1]/10 hover:text-[#6366F1] hover:border-[#6366F1]/30 disabled:opacity-30'}`}>
                  {isAnalyzing ? "Processing Transcript..." : "End & Save to Memory Log"}
                </button>
              </div>
            </div>
          )}
          <button onClick={() => setIsChatOpen(!isChatOpen)} className={`w-14 h-14 rounded-full flex items-center justify-center shadow-[0_8px_30px_rgba(0,0,0,0.3)] transition-all duration-500 hover:scale-110 active:scale-95 ${theme === 'dark' ? 'bg-gradient-to-tr from-[#5EEAD4] to-[#A78BFA] text-[#0F1117]' : 'bg-gradient-to-tr from-[#6366F1] to-[#22D3EE] text-white'}`}>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen p-4 sm:p-6 lg:p-8">
        {token ? (
          <div className="w-full max-w-3xl animate-fade-in-up pb-20 mt-12 sm:mt-0">
            ...
            {profileData ? (
              <>
                <div className={`p-6 sm:p-10 rounded-3xl transition-all duration-500 mb-10 relative z-20 ${theme === 'dark' ? 'bg-[#161A23] border border-[#2A3040] shadow-[0_8px_30px_rgba(0,0,0,0.45)]' : 'bg-white border border-[#E3E6EF] shadow-[0_8px_30px_rgba(0,0,0,0.04)]'}`}>
                  <div className="flex justify-between items-start mb-10">
                    <h2 className="text-3xl sm:text-4xl font-light mb-2">Welcome back, <span className="font-medium bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 to-purple-500 dark:from-[#5EEAD4] dark:to-[#A78BFA]">{getDisplayName()}</span>.</h2>
                    <button onClick={handleLogout} className={`p-2 rounded-lg transition-colors ${theme === 'dark' ? 'text-[#6B7280] hover:bg-[#1D2230] hover:text-white' : 'text-[#9CA3AF] hover:bg-[#F1F3F9] hover:text-[#1F2937]'}`} title="Disconnect"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg></button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div className={`p-5 rounded-2xl transition-colors duration-300 ${theme === 'dark' ? 'bg-[#0F1117] border border-[#1D2230]' : 'bg-[#F7F8FB] border border-[#F1F3F9]'}`}>
                       ...
                       <div className="flex flex-wrap gap-1 mt-2">{getLifeGrid()}</div>
                  ...
                  <div className="relative z-30">
                    <textarea value={reflectionInput} onChange={(e) => setReflectionInput(e.target.value)} placeholder="What is on your mind today? Type your reflection here..." disabled={isAnalyzing} className={`w-full min-h-[160px] p-6 rounded-2xl resize-y transition-all duration-500 outline-none font-['Source_Serif_4'] text-lg ${theme === 'dark' ? 'bg-[#0F1117] border border-[#2A3040] text-[#E6EAF2] placeholder-[#4B5563] focus:border-[#5EEAD4]/50 focus:shadow-[0_0_20px_rgba(94,234,212,0.05)]' : 'bg-[#F7F8FB] border border-[#E3E6EF] text-[#1F2937] placeholder-[#9CA3AF] focus:border-[#6366F1]/50 focus:shadow-[0_4px_20px_rgba(99,102,241,0.05)]'}`} />
                    <div className="flex justify-end mt-4">
                      <button onClick={() => submitReflection()} disabled={isAnalyzing || !reflectionInput.trim()} className={`px-6 py-3 rounded-xl font-medium tracking-wide transition-all duration-500 flex items-center gap-2 ${isAnalyzing ? 'opacity-70 cursor-wait' : 'hover:scale-105 active:scale-95 cursor-pointer'} ${theme === 'dark' ? 'bg-gradient-to-r from-[#5EEAD4] to-[#A78BFA] text-[#0F1117] hover:shadow-[0_0_20px_rgba(94,234,212,0.3)]' : 'bg-gradient-to-r from-[#6366F1] to-[#22D3EE] text-white hover:shadow-[0_8px_20px_rgba(99,102,241,0.3)]'} disabled:grayscale disabled:hover:scale-100 disabled:shadow-none`}>
                        {isAnalyzing ? <><div className="w-4 h-4 rounded-full border-2 border-[#0F1117] dark:border-white border-t-transparent animate-spin"></div>Analyzing Nexus...</> : 'Process Reflection'}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                {reflectionsHistory.length >= 2 && (
                  <div className="w-full mt-12 mb-12 relative z-20">
                     ...
                     {analysisData && !isAnalysisLoading && (
                       <div className="flex flex-col gap-6 animate-fade-in-up">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                             <div className={`p-6 rounded-2xl border-l-4 border-l-emerald-500 ${theme === 'dark' ? 'bg-[#161A23]/80 border-t border-r border-b border-[#2A3040]' : 'bg-white border-t border-r border-b border-[#E3E6EF] shadow-sm'}`}>
                                <span className="block text-xs uppercase tracking-widest font-medium text-emerald-500 mb-2">Strengths</span>
                                <p className={`text-sm leading-relaxed ${theme === 'dark' ? 'text-[#E6EAF2]' : 'text-[#1F2937]'}`} dangerouslySetInnerHTML={formatMarkdownText(analysisData.swot.strengths)} />
                             </div>
                             <div className={`p-6 rounded-2xl border-l-4 border-l-orange-500 ${theme === 'dark' ? 'bg-[#161A23]/80 border-t border-r border-b border-[#2A3040]' : 'bg-white border-t border-r border-b border-[#E3E6EF] shadow-sm'}`}>
                                <span className="block text-xs uppercase tracking-widest font-medium text-orange-500 mb-2">Weaknesses</span>
                                <p className={`text-sm leading-relaxed ${theme === 'dark' ? 'text-[#E6EAF2]' : 'text-[#1F2937]'}`} dangerouslySetInnerHTML={formatMarkdownText(analysisData.swot.weaknesses)} />
                             </div>
                             <div className={`p-6 rounded-2xl border-l-4 border-l-cyan-500 ${theme === 'dark' ? 'bg-[#161A23]/80 border-t border-r border-b border-[#2A3040]' : 'bg-white border-t border-r border-b border-[#E3E6EF] shadow-sm'}`}>
                                <span className="block text-xs uppercase tracking-widest font-medium text-cyan-500 mb-2">Opportunities</span>
                                <p className={`text-sm leading-relaxed ${theme === 'dark' ? 'text-[#E6EAF2]' : 'text-[#1F2937]'}`} dangerouslySetInnerHTML={formatMarkdownText(analysisData.swot.opportunities)} />
                             </div>
                             <div className={`p-6 rounded-2xl border-l-4 border-l-indigo-500 ${theme === 'dark' ? 'bg-[#161A23]/80 border-t border-r border-b border-[#2A3040]' : 'bg-white border-t border-r border-b border-[#E3E6EF] shadow-sm'}`}>
                                <span className="block text-xs uppercase tracking-widest font-medium text-indigo-500 mb-2">Threats</span>
                                <p className={`text-sm leading-relaxed ${theme === 'dark' ? 'text-[#E6EAF2]' : 'text-[#1F2937]'}`} dangerouslySetInnerHTML={formatMarkdownText(analysisData.swot.threats)} />
                             </div>
                          </div>

                          {/* WWW & EBI GRID */}
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                             <div className={`p-6 rounded-2xl border-l-4 border-l-teal-400 ${theme === 'dark' ? 'bg-[#161A23]/80 border-t border-r border-b border-[#2A3040]' : 'bg-white border-t border-r border-b border-[#E3E6EF] shadow-sm'}`}>
                                <span className="block text-xs uppercase tracking-widest font-medium text-teal-400 mb-2">What Went Well</span>
                                <p className={`text-sm leading-relaxed ${theme === 'dark' ? 'text-[#E6EAF2]' : 'text-[#1F2937]'}`} dangerouslySetInnerHTML={formatMarkdownText(analysisData.www_ebi.what_went_well)} />
                             </div>
                             <div className={`p-6 rounded-2xl border-l-4 border-l-violet-400 ${theme === 'dark' ? 'bg-[#161A23]/80 border-t border-r border-b border-[#2A3040]' : 'bg-white border-t border-r border-b border-[#E3E6EF] shadow-sm'}`}>
                                <span className="block text-xs uppercase tracking-widest font-medium text-violet-400 mb-2">Even Better If</span>
                                <p className={`text-sm leading-relaxed ${theme === 'dark' ? 'text-[#E6EAF2]' : 'text-[#1F2937]'}`} dangerouslySetInnerHTML={formatMarkdownText(analysisData.www_ebi.even_better_if)} />
                             </div>
                          </div>

                          {/* 5 WHYS TIMELINE */}
                          <div className={`p-6 rounded-2xl ${theme === 'dark' ? 'bg-[#161A23]/80 border border-[#2A3040]' : 'bg-white border border-[#E3E6EF] shadow-sm'}`}>
                             <span className={`block text-xs uppercase tracking-widest font-medium mb-6 ${theme === 'dark' ? 'text-[#A4A9B6]' : 'text-[#9CA3AF]'}`}>5 Whys Root Cause Analysis</span>
                             <div className={`flex flex-col gap-5 pl-4 border-l-2 border-dashed ${theme === 'dark' ? 'border-[#5EEAD4]/30' : 'border-[#6366F1]/30'}`}>
                                {analysisData.five_whys.map((why: string, idx: number) => (
                                   <div key={idx} className="relative">
                                      <span className={`absolute -left-[23px] top-1.5 w-2.5 h-2.5 rounded-full ${theme === 'dark' ? 'bg-[#5EEAD4] shadow-[0_0_8px_rgba(94,234,212,0.6)]' : 'bg-[#6366F1] shadow-[0_0_8px_rgba(99,102,241,0.6)]'}`}></span>
                                      <p className={`text-sm leading-relaxed ${theme === 'dark' ? 'text-[#E6EAF2]' : 'text-[#1F2937]'}`} dangerouslySetInnerHTML={formatMarkdownText(why)} />
                                   </div>
                                ))}
                             </div>
                          </div>
                          ...
                          <div className="mt-4 mb-4 text-center">
                             <p className={`font-['Source_Serif_4'] text-xl italic ${theme === 'dark' ? 'text-[#6B7280]' : 'text-[#9CA3AF]'}`}>
                                "{analysisData.quote}"
                             </p>
                          </div>
                       </div>
                  ...
                  <div className="w-full mt-8 animate-fade-in-up relative z-20">
                    ...
                    <div className="flex flex-col gap-6">
                      {reflectionsHistory.slice(0, visibleCount).map((ref) => {
                        ...
                        return (
                        <div key={ref.id} className={`p-6 sm:p-8 rounded-3xl transition-all duration-300 relative group ${theme === 'dark' ? 'bg-[#161A23] border border-[#2A3040] hover:border-[#5EEAD4]/30' : 'bg-white border border-[#E3E6EF] shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:border-[#6366F1]/30'}`}>
                          {isToday && (
                            <div className={`absolute top-6 right-6 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${theme === 'dark' ? 'text-[#6B7280]' : 'text-[#9CA3AF]'}`}>
                              {editingRefId !== ref.id && <button onClick={() => { setEditingRefId(ref.id); setEditContent(ref.content); }} className="p-2 hover:text-[#5EEAD4] transition-colors" title="Edit"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg></button>}
                              <button onClick={() => handleDeleteReflection(ref.id)} className="p-2 hover:text-rose-500 transition-colors" title="Delete"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                            </div>
                          )}

                          <div className="flex items-center gap-3 mb-4 opacity-80">
                            {ref.mood && <span className="text-xl">{getMoodIcon(ref.mood)}</span>}
                            <span className={`text-xs uppercase tracking-widest ${theme === 'dark' ? 'text-[#6B7280]' : 'text-[#9CA3AF]'}`}>{refDate.toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</span>
                          </div>
                          
                          {editingRefId === ref.id ? (
                            <div className="mb-6">
                              <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} className={`w-full min-h-[100px] p-4 rounded-xl resize-y outline-none font-['Source_Serif_4'] text-lg ${theme === 'dark' ? 'bg-[#0F1117] border border-[#5EEAD4]/50 text-[#E6EAF2]' : 'bg-[#F7F8FB] border border-[#6366F1]/50 text-[#1F2937]'}`} />
                            ...
                            <p className="font-['Source_Serif_4'] text-lg leading-relaxed mb-6 whitespace-pre-wrap pr-10" dangerouslySetInnerHTML={formatMarkdownText(ref.content)}></p>
                          )}
                          ...
                          {ref.ai_insight && (
                            <div className={`p-5 rounded-2xl relative overflow-hidden ${theme === 'dark' ? 'bg-[#0F1117]/80' : 'bg-[#F7F8FB]'}`}>
                              ...
                              <span className={`block text-[10px] uppercase tracking-widest font-medium mb-3 ${theme === 'dark' ? 'text-[#5EEAD4]' : 'text-[#6366F1]'}`}>Nexus Insight</span>
                              <p className={`text-sm leading-relaxed mb-4 ${theme === 'dark' ? 'text-[#A4A9B6]' : 'text-[#4B5563]'}`} dangerouslySetInnerHTML={formatMarkdownText(insightData.content)}></p>
                              
                              {insightData.tags.length > 0 && (
                                <div className="flex flex-wrap gap-2 mt-2">
                                  {insightData.tags.map((tag, idx) => (
                                    <span key={idx} className={`text-xs px-3 py-1 rounded-full font-medium tracking-wide border ${theme === 'dark' ? 'bg-[#5EEAD4]/10 text-[#5EEAD4] border-[#5EEAD4]/20' : 'bg-[#6366F1]/10 text-[#6366F1] border-[#6366F1]/20'}`}>{tag}</span>
                                  ))}
                                </div>
                              )}
                            </div>
                    ...
                    {visibleCount < reflectionsHistory.length && (
                      <div className="mt-8 text-center">
                        <button onClick={() => setVisibleCount(v => v + 2)} className={`px-6 py-3 rounded-full text-sm font-medium tracking-wider transition-all duration-300 border ${theme === 'dark' ? 'border-[#2A3040] text-[#A4A9B6] hover:bg-[#1D2230] hover:text-[#5EEAD4] hover:border-[#5EEAD4]' : 'border-[#E3E6EF] text-[#6B7280] hover:bg-[#F1F3F9] hover:text-[#6366F1] hover:border-[#6366F1]'}`}>Unlock Deeper Memory</button>
                      </div>
                    )}
                  </div>
                )}
          ...
          <div className="w-full max-w-md animate-fade-in-up relative z-20">
             <div className="text-center mb-12">
                ...
                <p className={`font-light tracking-wide ${theme === 'dark' ? 'text-[#A4A9B6]' : 'text-[#6B7280]'}`}>{theme === 'dark' ? 'Deep night sky, quiet thinking.' : 'Sunlight, calm thinking, elegant.'}</p>
             </div>
            
            <div className={`p-8 sm:p-10 rounded-3xl transition-all duration-500 ${theme === 'dark' ? 'bg-[#161A23] border border-[#2A3040] shadow-[0_8px_30px_rgba(0,0,0,0.45)]' : 'bg-white border border-[#E3E6EF] shadow-[0_10px_40px_rgba(0,0,0,0.06)]'}`}>
              <h2 className="text-2xl font-light mb-8 text-center tracking-wide">{isLoginView ? "Welcome Back" : "Begin Your Journey"}</h2>
              {errorMsg && <div className={`p-4 mb-8 rounded-xl text-sm animate-fade-in-up ${theme === 'dark' ? 'bg-rose-900/20 text-rose-400 border border-rose-900/50' : 'bg-rose-50 text-rose-600 border border-rose-100'}`}>{errorMsg}</div>}
              ...
              <div className="mt-8 text-center">
                <p className={`text-sm ${theme === 'dark' ? 'text-[#A4A9B6]' : 'text-[#6B7280]'}`}>{isLoginView ? "New to the Nexus? " : "Returning traveler? "} <button type="button" onClick={() => { setIsLoginView(!isLoginView); setErrorMsg(""); }} className={`font-medium transition-colors duration-300 ml-1 ${theme === 'dark' ? 'text-[#5EEAD4] hover:text-white' : 'text-[#6366F1] hover:text-[#1F2937]'}`}>{isLoginView ? "Initialize here." : "Access here."}</button></p>
              </div>
            </div>
          </div>
        )}
